...
cs0 = map0.imshow(ma.masked_equal(smap_sm_gridded_centralUS,0.0),origin='upper',interpolation='nearest',cmap=cmap,norm=norm, vmin=0.0, vmax=0.3)
...
ax0.text(0.2, 0.85, '(a)', verticalalignment='bottom', horizontalalignment='right', transform=ax0.transAxes,fontsize=20)
...
cs1 = map1.imshow(ma.masked_equal(ctrl_h2osoi_zero0[0,],0.0),origin='upper',interpolation='nearest',cmap=cmap,norm=norm, vmin=0.0, vmax=0.3)
...
ax1.text(0.2, 0.85, '(b)', verticalalignment='bottom', horizontalalignment='right', transform=ax1.transAxes,fontsize=20)
...
cs2 = map2.imshow(ma.masked_equal(diff,0.0),origin='upper',interpolation='nearest',cmap=cmap,norm=norm, vmax=0.0,vmin=-0.09)
...
plt.title('SMAP_kf - Control ' + str(YR) + str(MON).zfill(2),fontsize=15)
cbar_ax2 = fig.add_axes([0.67, 0.15, 0.2, 0.03])  
cb2 = fig.colorbar(cs2, cax=cbar_ax2, cmap=cmap, norm=norm, ticks=bounds_b, boundaries=bounds_b,spacing='uniform',orientation='horizontal', extend='min')
...
ax2.text(0.2, 0.85, '(c)', verticalalignment='bottom', horizontalalignment='right', transform=ax2.transAxes,fontsize=20)


plt.tight_layout(pad=0, w_pad=0, h_pad=0)


savefig(figDIR + 'Fig2_SMAP_CLMctrl_CLMSMAPKF_centralUSA_5cmSM_' + str(YEAR) + '-' + str('%02d'%(MONTH) ) + '.png', dpi=600, bbox_inches='tight')
plt.close()
