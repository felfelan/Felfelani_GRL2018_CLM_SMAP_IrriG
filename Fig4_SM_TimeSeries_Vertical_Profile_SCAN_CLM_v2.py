...
counter = 0	 
gs1 = gridspec.GridSpec(5, 1, height_ratios=[1,1,1,1,1]) # the middle row is to place the colorbar axes
#gs1.update(bottom=0.6, top=0.98)

fig = plt.figure(num=1, figsize=(15,40)) #figsize: w,h
ax = []
#%%======================================================================= read SCAN data
...
for cc in range(len(latseries)):
    ...
    df_SMAP_KF_h2osoi_5cmT = pd.DataFrame(columns=['Date','SMAP_KF_h2osoi_5cm_' + st_names[cc]])
...
#    df_JJA['Date'] = df_JJA['Date'].dt.strftime('%Y-%m-%d') #change the time format; removing the time 00:00:00 from Xaxis ticks labels
    ax[-1] = df_JJA.plot.scatter(x='ind',y=st_names[cc] ,ax = ax[-1],s = 6 , c='black',marker="s",\
                           xticks=df_JJA.index[0:len(df_JJA.index):45],\
                           title='Top 5cm Soil Moisture for JJA ' ,label='SCAN/CRN Observations')
...
#    ax[-1].scatter(df_JJA.index, df_JJA[clmsmapKF_col].copy().mean(1),s = 5, c='forestgreen',label='CLM SMAP_KF')
    ...
    ax[-1].scatter(df_JJA.index, df_JJA['ctrl_h2osoi_5cm_' + st_names[cc]].copy(),s = 5 , c='tomato',label='CLM Control')
    ax[-1].scatter(df_JJA.index, df_JJA['SMAP_h2osoi_5cm_' + st_names[cc]].copy(),s = 5, c='dodgerblue',label='CLM SMAP_Raw')
    ax[-1].scatter(df_JJA.index, df_JJA['SMAP_KF_h2osoi_5cm_' + st_names[cc]].copy(),s = 5, c='forestgreen',label='CLM SMAP_KF')
    ...
    # plt.ylim(0,0.5)
    if cc == 0 :
        plt.title('Top 5cm Soil Moisture for JJA - ' +  st_names[cc],fontsize=10)
    else:
        plt.title(st_names[cc],fontsize=10)        

savefig( figDIR + 'Fig4_SM_timeseries' + sDate_T + eDate_T + 'v3.png', bbox_inches='tight', dpi=600 )
savefig( figDIR + 'Fig4_SM_timeseries' + sDate_T + eDate_T + 'v3.pdf', bbox_inches='tight')
