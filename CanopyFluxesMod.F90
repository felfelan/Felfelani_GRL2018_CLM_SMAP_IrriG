...
! !REVISION HISTORY:    
!
!     Created By:   MM/DD/YYYY, Mariana Vertenstein
!     Update 1:     04/25/2005, Peter Thornton
!                               replaced old Stomata subroutine with 
!                               what used to be called StomataCN, as 
!                               part of migration to new sun/shade 
!                               algorithms.             
!
!     Update 2:	    08/01/2018, Farshid Felfelani
!                               A novel irrigation scheme is implemented by setting
!                               the target soil moisture based on SMAP satelllite
!                               observations of top 5-cm soil moisture. SMAP data
!                               are first biass corrected and then assimilated
!                               using a 1-D Kalman Filter.
!
!                               Reference: Felfelani et al. (2018) GRL
...
! !USES:
   ...
   use decompMod      , only: ldecomp, get_proc_global, get_proc_bounds, get_clump_bounds,get_proc_clumps
...
! !USES:
    ...
    use clm_time_manager   , only : get_step_size, get_prev_date,get_prev_date_FFmod 
    use clm_varpar         , only : nlevgrnd, nlevsno, nsmap
...
!
   ...
   real(r8), pointer :: h2osoi_vol_tar(:,:)      ! volumetric soil water (0<=h2osoi_vol<=watsat) [m3/m3]
   real(r8), pointer :: h2osoi_vol_tarSMAP(:,:)  ! volumetric soil water (0<=h2osoi_vol<=watsat) [m3/m3]
...
#endif
   real(r8), pointer :: irrig_rate(:)          ! current irrigation rate [mm/s]; updated with SMAP (Farshid Felfelani)
   real(r8) :: irrig_rate_orig                 ! CLM original: current irrigation rate [mm/s]                 
   real(r8) :: irrig_rate_smap  
...
!
   ...
   !added by Farshid Felfelani for SMAP
   real(r8), pointer :: cellsmap(:,:) ! column SMAP soil moisture
   real(r8), pointer :: smapg(:,:)    ! gridcell SMAP soil moisture    
   ...
   integer  :: doy                      ! Farshid Felfelani; day of the year 1~365
   ...
   real(r8) :: vol_liq_so               ! partial volume of liquid water in layer; updated with SMAP (Farshid Felfelani)
   real(r8) :: vol_liq_so_orig          ! CLM original: partial volume of liquid water in layer for which smp_node = smpso
   real(r8) :: vol_liq_so_smap
   real(r8) :: R_error, smap_noise, xhat0, P0, &
               xhatminus, Pminus, Y_p, gain, xhat, Phat    
   integer, parameter  :: n_itr=30
   integer  :: nn, ii, jj, nnn, idx, i1
   real(r8), dimension(n_itr) :: Y_p_array 
   integer  :: index_arr1(30), index_arr2(80)
   real(r8) :: h2osoi_liq_so            ! liquid water corresponding to vol_liq_so for this layer [kg/m2]; updated with SMAP (Farshid Felfelani)
   real(r8) :: h2osoi_liq_so_smap 
   real(r8) :: h2osoi_liq_so_orig       ! CLM original: liquid water corresponding to vol_liq_so for this layer [kg/m2]

   real(r8) :: h2osoi_liq_sat           ! liquid water corresponding to eff_porosity for this layer [kg/m2]
   real(r8) :: deficit                  ! updated with SMAP (Farshid Felfelani)
   real(r8) :: deficit_smap 
   real(r8) :: deficit_orig             ! CLM original: difference between desired soil moisture level for this layer and current soil moisture level [kg/m2]
   ...
   integer :: nclumps           ! proc clump index
   integer :: begp_cl, endp_cl  ! clump beg and end pft indices
   integer :: begc_cl, endc_cl  ! clump beg and end column indices
   integer :: begl_cl, endl_cl  ! clump beg and end landunit indices
   integer :: begg_cl, endg_cl  ! clump beg and end gridcell indices
   real(4) :: randval   
!------------------------------------------------------------------------------
   ...
   h2osoi_vol_tar         => cws%h2osoi_vol_tar
   h2osoi_vol_tarSMAP     => cws%h2osoi_vol_tarSMAP
...
!   irrig_rate_orig      => cps%irrig_rate_orig
   ...
   call get_prev_date_FFmod(yr, mon, day, time,doy)  ! get time as of beginning of time step
   do f = 1, fn
      p = filterp(f)
      c = pcolumn(p)
      g = pgridcell(p)
   ...
   call get_clump_bounds (nclumps, begg_cl, endg_cl, begl_cl, endl_cl, begc_cl, endc_cl, begp_cl, endp_cl)
      ...
      do f = 1, fn
         ...
         if (check_for_irrig(p) .and. .not. frozen_soil(c)) then
            ...
            if (t_soisno(c,j) <= SHR_CONST_TKFRZ) then
               ...
               vol_liq_so_orig   = eff_porosity * (-smpso(ivt(p))/sucsat(c,j))**(-1/bsw(c,j))
               if (cellsmap(c,doy) == 0._r8) then
                    ! If the current day is zero (no SMAP data) but one day before of after has data 
                    if ((cellsmap(c,doy + 1) /= 0._r8 .or. cellsmap(c,doy - 1) /= 0._r8) .and. doy/=1 .and. doy/=365)then
                        if (j == 1 .or. j == 2) then
                            vol_liq_so_smap   = max(cellsmap(c,doy + 1),cellsmap(c,doy - 1))
                        else
                            vol_liq_so_smap   = max(cellsmap(c,doy + 1),cellsmap(c,doy - 1)) + &
                                                max(cellsmap(c,doy + 1),cellsmap(c,doy - 1)) * &
                                                (1._r8-(1._r8-h2osoi_vol(c,j)/watsat(c,j))**bsw(c,j))
                        end if 
                        ! day should be day of year from 1 to 365 not day of month 1 to 31!!!!
                        ! I use doy instead of Day
                    else
                        ! If the SMAP data of 3 successive days are zero, uses the CLM Default Parameterization
                        vol_liq_so_smap   = eff_porosity * (-smpso(ivt(p))/sucsat(c,j))**(-1/bsw(c,j))
                    endif
               else
                    if (j == 1 .or. j == 2) then
                        vol_liq_so_smap   = cellsmap(c,doy)
                    else
                        vol_liq_so_smap   = cellsmap(c,doy) + cellsmap(c,doy) * &
                                            (1._r8-(1._r8-h2osoi_vol(c,j)/watsat(c,j))**bsw(c,j))
                    end if
               end if

               !! The Kalman Filter Section Begins
               R_error = 0.09_r8
               smap_noise = 0.015_r8  !white noise of SMAP

			   ! The first guess for entering the Kalman Filter is the same combination of minimum SM and Maximum SM as the CLM Default scheme
               xhat0 = vol_liq_so_orig + irrig_factor*(eff_porosity - vol_liq_so_orig) 

               Y_p_array(1:n_itr) = vol_liq_so_smap
               !! First 15 values are based on temporal values of exact cell but in 			   
               ...
               idx = 1 
               if (doy .ge. 16 .and. doy .le. 350) then
                  do nn = 2,15 ! the first value is the exact cell at exat day
                     do nnn = idx,size(index_arr1)
                        if (cellsmap(c,doy + index_arr1(nnn)) .eq. 0._r8) then
                            cycle
                        end if 
                        if (j == 1 .or. j == 2) then
                            Y_p_array(nn) = cellsmap(c,doy + index_arr1(nnn)) 
                        else
                            Y_p_array(nn) = cellsmap(c,doy + index_arr1(nnn)) + cellsmap(c,doy + index_arr1(nnn)) * &
                                                                                (1._r8-(1._r8-h2osoi_vol(c,j)/watsat(c,j))**bsw(c,j))
                        end if
                        exit
                        ! if in 20 consecutive SMAP data there are not 14 unzero values, it is around winter and the SM is probably zero
                     end do
                     idx = nnn + 1
                  end do
               end if
               ...
               do ii = 1,40
                  index_arr2(2*ii-1) = -ii
                  index_arr2(2*ii)   = ii
               end do
               idx = 1
               if (g .gt. begg + 40 .and. g .lt. endg - 40) then
                  do jj = nn, n_itr
                     do nnn = idx,size(index_arr2)
                        if (ANY(Y_p_array .eq. smapg(g + index_arr2(nnn),doy)) .or. smapg(g + index_arr2(nnn),doy) .eq. 0._r8 .or. &
                                 sqrt((grc%latdeg(g)-grc%latdeg(g + index_arr2(nnn)))**2.00_r8 + &
                                     (grc%londeg(g)-grc%londeg(g + index_arr2(nnn)))**2.00_r8) .ge. 1.5_r8) then
                                 cycle
                        end if 
                        if (j == 1 .or. j == 2) then
                            Y_p_array(jj) = smapg(g + index_arr2(nnn),doy)					  
                        else
                            Y_p_array(jj) = smapg(g + index_arr2(nnn),doy) + smapg(g + index_arr2(nnn),doy) * (1-(1-h2osoi_vol(c,j)/watsat(c,j))**bsw(c,j))   
                        end if
                        exit
                     end do
                     idx = nnn + 1
               ...
               !! P0 = 1.0_r8 !not a good initial guess for model error estimate; it is too high
               !! if P0 is high (P0 = 1) xhat rapidly converges to vol_liq_so_smap because its error is 0.09^2 = 0.0081 and gain becomes close to 1
               P0 = 0.2_r8
               do nn = 1, n_itr
                  if (nn == 1) then
                     xhatminus = xhat0
                     ...
                     if (randval .le. 0.5_r8) then 
                        randval = 0.0_r8
                     else if (randval .gt. 0.5_r8) then 
                        randval = 1.0_r8
                     end if
                     ! Scaling random numbers: rand() is always between 0 and 1 to scale it between [- smap_noise , smap_noise): (rand(0)*(y-x))+x                   
                     Y_p = max(Y_p_array(nn) + dble(randval) * (smap_noise * 2.0_r8) - smap_noise , 0.0_r8)
                     gain = Pminus/(Pminus + R_error)
                     xhat = xhatminus + gain * (Y_p-xhatminus)
                     Phat = (1 - gain) * Pminus
                  else
                     xhatminus = xhat
                     Pminus = Phat
               ...
               !! The Kalman Filter Section Ends

			   h2osoi_vol_tar(c,j) = max(xhat0, 0._r8)
			   h2osoi_vol_tarSMAP(c,j) = max(xhat, 0._r8)
			   
			   
			   h2osoi_liq_so  = vol_liq_so_orig * denh2o * dz(c,j)
               h2osoi_liq_so_smap = xhat * denh2o * dz(c,j)
               ...
               deficit_smap    = max(h2osoi_liq_so_smap - h2osoi_liq(c,j), 0._r8)

               ! Add deficit to irrig_rate, converting units from mm to mm/sec

               irrig_rate_orig  = deficit/(dtime*irrig_nsteps_per_day)
               irrig_rate_smap  = deficit_smap/(dtime*irrig_nsteps_per_day)
               irrig_rate(c) = irrig_rate(c) + min(irrig_rate_orig,irrig_rate_smap) 
