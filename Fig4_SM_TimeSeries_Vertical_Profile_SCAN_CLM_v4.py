...
X_024_DIR = '/mnt/scratch/felfelan/CESM_simulations/X_024_centralUSA_ICRUCLM45BGCCROPNLDAS_SMAPdailyirr/'
...
SCAN_CRN_VP_DIR = '/mnt/home/felfelan/CESM_DIRs/MyAnalysis/scan_uscrn_snotel/CRN_SCAN_Selected_Stations/Vertical_Profile/'
...
gs1.update(bottom=0.56, top=0.95,wspace=0.12, hspace = 0.10)
...
gs2.update(bottom=0.05, top=0.46, wspace=0.15)

fig = plt.figure(num=1, figsize=(25,20)) #figsize: w,h
ax = []
#%%======================================================================= read SCAN data
...
#    ax[-1].scatter(df_JJA.index, df_JJA[clmsmapKF_col].copy().mean(1),s = 5, c='forestgreen',label='CLM SMAP_KF')  <
    ...
    sc2 = ax[-1].scatter(df_JJA.index, df_JJA['ctrl_h2osoi_5cm_' + st_names[cc]].copy(),s = 7 , c='tomato',marker="<",label='CLM Control')
    sc3 = ax[-1].scatter(df_JJA.index, df_JJA['SMAP_h2osoi_5cm_' + st_names[cc]].copy(),s = 7, c='dodgerblue',marker="s",label='CLM SMAP_raw')
    sc4 = ax[-1].scatter(df_JJA.index, df_JJA['SMAP_KF_h2osoi_5cm_' + st_names[cc]].copy(),s = 7, c='forestgreen',marker="h",label='CLM SMAP_kf_BC/SMAP_kf')
    ...
    ln4 = ax[-1].plot(df_JJA.index, df_JJA['SMAP_KF_h2osoi_5cm_' + st_names[cc]].copy(),linewidth=0.5,c='forestgreen',label='CLM SMAP_kf_BC/SMAP_kf')
    ...
    if cc == 3 or cc ==2:    
        ax[-1].set_xticklabels(df_JJA.Date[0:len(df_JJA.index):30], fontsize=15, rotation=-45)
...
#        ax[-1].legend_.remove()
    ...
    if cc ==2:
        ...
        ax[-1].yaxis.set_label_coords(-0.08, 1.0) 
...
axe = fig.add_axes([0.11,0.45,0.8,0.03])	
pie = plt.pie([1,1])
...
lgnd = plt.legend(handles=[sc1,sc2,sc3,sc4,ba1],bbox_to_anchor=(0., 1.02, 1., .102),ncol=5, mode="expand", borderaxespad=0., edgecolor='k',prop={'size':15})
lgnd.legendHandles[0]._sizes = [40]  # size of the scatter in the legend
lgnd.legendHandles[1]._sizes = [40]
lgnd.legendHandles[2]._sizes = [40]
lgnd.legendHandles[3]._sizes = [40]
...
#%%======================================================================= Read SMAP data
	...
	# SMAP_monthly = np.load(SMAP_File)[:,smapxy(lati,long)[1],smapxy(lati,long)[0]].mean()
...
#%%======================================================================= Plotting the Whole timeseries
	...
	ax[-1].scatter(SMAP_monthly,0.025 ,s = 30 , c='black',marker='*', facecolors='none',label='SMAP')

	if 'SCAN' in tables:	
		ax[-1].scatter(array_scan, array((0.05,0.1,0.2,0.5,1))  ,s = 7 , c='black',marker='s', facecolors='none',label='SCAN')
	else:
		ax[-1].scatter(array_scan, array((0.05,0.1,0.2,0.5,1))  ,s = 7 , c='black',marker='s', facecolors='none',label='USCRN')

	ax[-1].plot(array_scan, array((0.05,0.1,0.2,0.5,1)),c='black', linestyle =':')
	
	ax[-1].scatter(array_ctrl, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038))  ,s = 7 , c='tomato',marker='<', facecolors='none',label='CLM_ctrl')
	ax[-1].plot(array_ctrl, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038)),c='tomato', linestyle =':')

	ax[-1].scatter(array_smap, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038))  ,s = 7 , c='dodgerblue',marker='o', facecolors='none',label='CLM SMAP_raw')
	ax[-1].plot(array_smap, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038)),c='dodgerblue', linestyle =':') 

	ax[-1].scatter(array_smapDailyCVEX, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038))  ,s = 7 , c='forestgreen',marker='h', facecolors='none',label='CLM SMAP_kf')
	...
	ax[-1].set_title(TABLE_Both, fontsize=15)
	if gs2counter == 0:
...
savefig( figDIR + 'Fig4_SM_timeseriesLHPA_' + sDate_T + eDate_T + '_Ave_SM_vertical_X021_X024_X024kf_Month' + AveMon + '_' + sDate_V + '_' + eDate_V + '_v4.png', bbox_inches='tight', dpi=600 )
# savefig( figDIR + 'Fig4_SM_timeseriesLPHA_' + sDate_T + eDate_T + '_Ave_SM_vertical_X021_X024_X024kf_Month' + AveMon + '_' + sDate_V + '_' + eDate_V + '_v4.pdf', bbox_inches='tight')

# savefig( figDIR + 'Fig4_SM_timeseriesLHPA_' + sDate_T + eDate_T + '_Ave_SM_vertical_X021_X024_X024kf_' + '_' + sDate_V + '_' + eDate_V + '_v4.png', bbox_inches='tight', dpi=600 )
