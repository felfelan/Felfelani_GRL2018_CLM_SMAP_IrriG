...
region = 'southernHPA'
lat_T1 = 325 # Southern HPA Lat/Lon; after doing flipud
lat_T2 = 334
lon_T1 = 259
lon_T2 = 289
...
counter = 0	 
gs1 = gridspec.GridSpec(1, 1) # the middle row is to place the colorbar axes
...
gs2.update(bottom=0.02, top=0.5, wspace=0.15)

fig = plt.figure(num=1, figsize=(15,13)) #figsize: w,h
ax = []
#%%======================================================================= read SCAN data
...
for file in sorted(os.listdir(SCAN_CRN_DIR)):
    if os.path.isfile(os.path.join(SCAN_CRN_DIR, file)) and 'SCAN' in file:  #list only files not directories
        print(file)
        nSTATION = nSTATION + 1
...
for i, tables in enumerate(my_dict):
  if 'SCAN' in tables:
    ...
    if i == 0:
        df_SCAN_T = df2.rename(columns={i:'{}_{}'.format(i, tables) for i in filter_col}).copy()
    else:
...
for file in sorted(os.listdir(CLM_ctrl_DIR)):
    if fnmatch.fnmatch(file, '*00000.nc') and int(file[47:51])>= sYR_T and int(file[47:51])<= eYR_T:
        ...
        if file[47:51] == str(sYR_T) and xx == 0:
            print('file0:',file)
            IRRG_h2osoi_dat1 = IRRG_h2osoi_dat01[:,lat_T1:lat_T2 + 1, lon_T1:lon_T2 + 1]
            IRRG_h2osoi_dat2 = IRRG_h2osoi_dat02[:,lat_T1:lat_T2 + 1, lon_T1:lon_T2 + 1]
            xx = 1

        else:
            print('file:',file)
            IRRG_h2osoi_dat1 = concatenate(( IRRG_h2osoi_dat1, IRRG_h2osoi_dat01[:,lat_T1:lat_T2 + 1, lon_T1:lon_T2 + 1] ))
            IRRG_h2osoi_dat2 = concatenate(( IRRG_h2osoi_dat2, IRRG_h2osoi_dat02[:,lat_T1:lat_T2 + 1, lon_T1:lon_T2 + 1] ))
	  

IRRG_h2osoi_dat = IRRG_h2osoi_dat1*(0.014201/0.041649) + IRRG_h2osoi_dat2*(0.027447/0.041649) 	

h2osoi_5cm = IRRG_h2osoi_dat.mean(2).mean(1)

df_ctrl_h2osoi_5cmT = pd.DataFrame(data=h2osoi_5cm[0:len(df2)])
df_ctrl_h2osoi_5cmT.rename(columns={0: 'ctrl_h2osoi_5cm'}, inplace=True)
df_T_noLeap = pd.concat([df_T_noLeap,df_ctrl_h2osoi_5cmT],axis=1)
...
for file in sorted(os.listdir(CLM_SMAP_daily_convex)):
    if fnmatch.fnmatch(file, '*00000.nc') and int(file[60:64])>= sYR_T and int(file[60:64])<= eYR_T:
        ...
        if file[60:64] == str(sYR_T) and xx==0:
            print('file0:',file)
            SMAP_IRRG_h2osoi_dat1 = SMAP_IRRG_h2osoi_dat01[:,lat_T1:lat_T2 + 1, lon_T1:lon_T2 + 1]
            SMAP_IRRG_h2osoi_dat2 = SMAP_IRRG_h2osoi_dat02[:,lat_T1:lat_T2 + 1, lon_T1:lon_T2 + 1]
            xx = 1

        else:
            print('file:',file)
            SMAP_IRRG_h2osoi_dat1 = concatenate(( SMAP_IRRG_h2osoi_dat1, SMAP_IRRG_h2osoi_dat01[:,lat_T1:lat_T2 + 1, lon_T1:lon_T2 + 1] ))
            SMAP_IRRG_h2osoi_dat2 = concatenate(( SMAP_IRRG_h2osoi_dat2, SMAP_IRRG_h2osoi_dat02[:,lat_T1:lat_T2 + 1, lon_T1:lon_T2 + 1] ))

SMAP_IRRG_h2osoi_dat = SMAP_IRRG_h2osoi_dat1*(0.014201/0.041649) + SMAP_IRRG_h2osoi_dat2*(0.027447/0.041649) 	

SMAP_h2osoi_5cm = SMAP_IRRG_h2osoi_dat.mean(2).mean(1)
df_smap_h2osoi_5cmT = pd.DataFrame(data=SMAP_h2osoi_5cm[0:len(df2)])
df_smap_h2osoi_5cmT.rename(columns={0: 'SMAP_h2osoi_5cm'}, inplace=True)
df_T_noLeap = pd.concat([df_T_noLeap,df_smap_h2osoi_5cmT],axis=1)
...
for file in sorted(os.listdir(CLM_SMAP_daily_KF)):
    if fnmatch.fnmatch(file, '*00000.nc') and int(file[63:67])>= sYR_T and int(file[63:67])<= eYR_T:
        ...
        if file[63:67] == str(sYR_T) and xx==0:
            print('file0:',file)
            SMAP_KF_IRRG_h2osoi_dat1 = SMAP_KF_IRRG_h2osoi_dat01[:,lat_T1:lat_T2 + 1, lon_T1:lon_T2 + 1]
            SMAP_KF_IRRG_h2osoi_dat2 = SMAP_KF_IRRG_h2osoi_dat02[:,lat_T1:lat_T2 + 1, lon_T1:lon_T2 + 1]
            xx = 1

        else:
            print('file:',file)
            SMAP_KF_IRRG_h2osoi_dat1 = concatenate(( SMAP_KF_IRRG_h2osoi_dat1, SMAP_KF_IRRG_h2osoi_dat01[:,lat_T1:lat_T2 + 1, lon_T1:lon_T2 + 1] ))
            SMAP_KF_IRRG_h2osoi_dat2 = concatenate(( SMAP_KF_IRRG_h2osoi_dat2, SMAP_KF_IRRG_h2osoi_dat02[:,lat_T1:lat_T2 + 1, lon_T1:lon_T2 + 1] ))

SMAP_KF_IRRG_h2osoi_dat = SMAP_KF_IRRG_h2osoi_dat1*(0.014201/0.041649) + SMAP_KF_IRRG_h2osoi_dat2*(0.027447/0.041649) 	

SMAP_KF_h2osoi_5cm = SMAP_KF_IRRG_h2osoi_dat.mean(2).mean(1)
df_SMAP_KF_h2osoi_5cmT = pd.DataFrame(data=SMAP_KF_h2osoi_5cm[0:len(df2)])
df_SMAP_KF_h2osoi_5cmT.rename(columns={0: 'SMAP_KF_h2osoi_5cm'}, inplace=True)
df_T_noLeap = pd.concat([df_T_noLeap,df_SMAP_KF_h2osoi_5cmT],axis=1)
...
df_JJA['Date2'] = df_JJA['Date'].dt.strftime('%Y-%m-%d') #change the time format; removing the time 00:00:00 from Xaxis ticks labels
ax[-1] = df_JJA.plot.scatter(x='ind',y='meanSCAN' ,ax = ax[-1],s = 6 , c='black',marker="s",\
                       xticks=df_JJA.index[0:len(df_JJA.index):45],\
                       title='Top 5cm Soil Moisture for JJA ' + region,label='SCAN/CRN Observations')
...
ax[-1].scatter(df_JJA.index, df_JJA.iloc[:,nSTATION + 1],s = 5 , c='tomato',label='CLM Control')
ax[-1].scatter(df_JJA.index, df_JJA.iloc[:,nSTATION + 2],s = 5, c='dodgerblue',label='CLM SMAP_Raw')
ax[-1].scatter(df_JJA.index, df_JJA.iloc[:,nSTATION + 3],s = 5, c='forestgreen',label='CLM SMAP_KF')

ax[-1].set_xticklabels(df_JJA.Date2[0:len(df_JJA.index):45], fontsize=10, rotation=-45)
...
legend_properties = {'size': 13}     # phont size
lgnd = plt.legend(loc='upper left',prop=legend_properties,ncol=3)
lgnd.legendHandles[0]._sizes = [60]  # size of the scatter in the legend
...
# plt.ylim(0,0.5)
plt.title('Top 5cm Soil Moisture for JJA - Southern HPA',fontsize=16)

#%%======================================================================= Vertical Profiles ==================================================================== 
nSTATION = 0 
for file in sorted(os.listdir(SCAN_CRN_DIR)):
    if os.path.isfile(os.path.join(SCAN_CRN_DIR, file)) and 'SCAN' not in file:  #list only files not directories; and only CRN files
        print(file)
        nSTATION = nSTATION + 1       
        # Reading from the header below
...
for i, tables in enumerate(my_dict):
	...
	if 'SCAN' in tables:
		...
		if i == 0:
			sum_df1 = df1.rename(columns={i:'{}'.format(tables[:-4]) for i in filter_col1}).copy()
			sum_df2 = df2.rename(columns={i:'{}'.format(tables[:-4]) for i in filter_col2}).copy()
			sum_df3 = df3.rename(columns={i:'{}'.format(tables[:-4]) for i in filter_col3}).copy()
			sum_df4 = df4.rename(columns={i:'{}'.format(tables[:-4]) for i in filter_col4}).copy()
			sum_df5 = df5.rename(columns={i:'{}'.format(tables[:-4]) for i in filter_col5}).copy()
		# originally it was columns={i:'{}_{}'.format(i,tables) for i in filter_col1}, it put the i (filter_col1) in the first {}
		# and put the tables (e,g,'SCAN_2083_Uapb_Campus_PB_AR.txt') in the second {}, so the name of the column got very lengthy
		# I just need the tables without '.txt' as the column title in df. So I changed it to above one. [:-4] is for removing '.txt'
		else:
			df1 = df1.rename(columns={i:'{}'.format(tables[:-4]) for i in filter_col1})
			sum_df1 = df1.merge(sum_df1, how='outer', on=['Date']) #, suffixes=('_{}'.format(tables), '_y'))
			# Use either 'suffix' for renaming or 'df1 = df1.rename()' or both...
			df2 = df2.rename(columns={i:'{}'.format(tables[:-4]) for i in filter_col2})
			sum_df2 = df2.merge(sum_df2, how='outer', on=['Date'])
			df3 = df3.rename(columns={i:'{}'.format(tables[:-4]) for i in filter_col3})
			sum_df3 = df3.merge(sum_df3, how='outer', on=['Date'])
			df4 = df4.rename(columns={i:'{}'.format(tables[:-4]) for i in filter_col4})
			sum_df4 = df4.merge(sum_df4, how='outer', on=['Date'])
			df5 = df5.rename(columns={i:'{}'.format(tables[:-4]) for i in filter_col5})
...
#        print(nt)
	...
	df_ctrl_h2osoi01['Date'] = pd.to_datetime(mcdate,format='%Y%m%d')
...
#%%======================================================================= read SMAP CLM data
	...
	df_smap_h2osoi01['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_smap_h2osoi02['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_smap_h2osoi1['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_smap_h2osoi2['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_smap_h2osoi3['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_smap_h2osoi4['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_smap_h2osoi5['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_smap_h2osoi6['Date'] = df_ctrl_h2osoi01['Date']
...
#%%======================================================================= read SMAP CLM X_024 daily, KF
	...
	for file in sorted(os.listdir(CLM_SMAP_daily_KF)):
		...
		if fnmatch.fnmatch(file, '*00000.nc') and int(file[63:67])>= sYR_V and int(file[63:67])<= eYR_V:
			SMAPdailyCVEX_IRRG_FILE = netcdf.netcdf_file(CLM_SMAP_daily_KF + file,'r')
	...
	df_SMAPdailyCVEX_h2osoi01['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_SMAPdailyCVEX_h2osoi02['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_SMAPdailyCVEX_h2osoi1['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_SMAPdailyCVEX_h2osoi2['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_SMAPdailyCVEX_h2osoi3['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_SMAPdailyCVEX_h2osoi4['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_SMAPdailyCVEX_h2osoi5['Date'] = df_ctrl_h2osoi01['Date']
	...
	df_SMAPdailyCVEX_h2osoi6['Date'] = df_ctrl_h2osoi01['Date']
...
#%%======================================================================= Plotting the Whole timeseries
	...
	if 'SCAN' in tables:
		sum_df1[tables[:-4]] = sum_df1[tables[:-4]]/100.00
		sum_df2[tables[:-4]] = sum_df2[tables[:-4]]/100.00
		sum_df3[tables[:-4]] = sum_df3[tables[:-4]]/100.00
		sum_df4[tables[:-4]] = sum_df4[tables[:-4]]/100.00
		sum_df5[tables[:-4]] = sum_df5[tables[:-4]]/100.00
	
	# Extarct just one month
	sum_df1_Mon = sum_df1[sum_df1.Date.str[5:7] == AveMon]
	sum_df2_Mon = sum_df2[sum_df2.Date.str[5:7] == AveMon]
	sum_df3_Mon = sum_df3[sum_df3.Date.str[5:7] == AveMon]
	sum_df4_Mon = sum_df4[sum_df4.Date.str[5:7] == AveMon]
	sum_df5_Mon = sum_df5[sum_df5.Date.str[5:7] == AveMon]    
	...
	ax[-1].scatter(SMAP_monthly,0.025 ,s = 20 , c='black',marker='*', facecolors='none',label='SMAP')

	if 'SCAN' in tables:	
		ax[-1].scatter(array_scan, array((0.05,0.1,0.2,0.5,1))  ,s = 5 , c='black',marker='o', facecolors='none',label='SCAN')
	else:
		ax[-1].scatter(array_scan, array((0.05,0.1,0.2,0.5,1))  ,s = 5 , c='black',marker='o', facecolors='none',label='USCRN')

	ax[-1].plot(array_scan, array((0.05,0.1,0.2,0.5,1)),c='black', linestyle =':')
	
	ax[-1].scatter(array_ctrl, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038))  ,s = 5 , c='tomato',marker='<', facecolors='none',label='CLM_ctrl')
	ax[-1].plot(array_ctrl, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038)),c='tomato', linestyle =':')

	ax[-1].scatter(array_smap, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038))  ,s = 5 , c='dodgerblue',marker='s', facecolors='none',label='CLM SMAP')
	ax[-1].plot(array_smap, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038)),c='dodgerblue', linestyle =':') 

	ax[-1].scatter(array_smapDailyCVEX, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038))  ,s = 5 , c='forestgreen',marker='h', facecolors='none',label='CLM SMAP KF')
	ax[-1].plot(array_smapDailyCVEX, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038)),c='forestgreen', linestyle =':') 	
	...
	ax[-1].set_title(TABLE_Both, fontsize=10)
	if gs2counter == 0:
		ax[-1].set_ylabel('Soil Depth (m)', fontsize=15, color='k')
		ax[-1].yaxis.set_label_coords(-0.16, -0.15) 
	if gs2counter == 6:			
		ax[-1].set_xlabel('Soil Moisture Content ' + r'($\mathrm{mm^3/mm^3}$)', fontsize=15, color='k')
...
savefig( figDIR + 'Fig4_SM_timeseriesLHPA' + sDate_T + eDate_T + '_Ave_SM_vertical_X022_MOD_CNVX_Month' + AveMon + '_' + sDate_V + eDate_V + 'v2.png', bbox_inches='tight', dpi=600 )
savefig( figDIR + 'Fig4_SM_timeseriesLPHA' + sDate_T + eDate_T + '_Ave_SM_vertical_X022_MOD_CNVX_Month' + AveMon + '_' + sDate_V + eDate_V + 'v2.pdf', bbox_inches='tight')
