...
gs2 = gridspec.GridSpec(5, 5, height_ratios=[1,1,1,1,1], width_ratios=[1,1,1,1,1]) # the middle row is to place the colorbar axes
gs2.update(bottom=0.14, top=0.96, wspace=0.05,hspace = 0.45)

fig = plt.figure(num=1, figsize=(8.0,9.9)) #figsize: w,h #0.9 of letter size: 8.5"x11"
ax = []
...
#%%======================================================================= Plotting the Whole timeseries
	...
	sc1 = ax[-1].scatter(SMAP_monthly,0.025 ,s = 20 , c='black',marker='*', facecolors='black',label='SMAP Satellite')

	if 'SCAN' in tables:	
		sc2 = ax[-1].scatter(array_scan, array((0.05,0.1,0.2,0.5,1))  ,s = 5 , c='black',marker='s', facecolors='black',label='Ground Observations')
	elif 'SNOTEL' in tables:	
		sc2 = ax[-1].scatter(array_scan, array((0.05,0.1,0.2,0.5,1))  ,s = 5 , c='black',marker='s', facecolors='black',label='Ground Observations')
	else:
		sc2 = ax[-1].scatter(array_scan, array((0.05,0.1,0.2,0.5,1))  ,s = 5 , c='black',marker='s', facecolors='black',label='Ground Observations')
	...
	sc3 = ax[-1].scatter(array_ctrl, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038))  ,s = 5 , c='tomato',marker='^', facecolors='tomato',label='CLM_ctrl')
	ax[-1].plot(array_ctrl, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038)),c='tomato', linestyle =':', linewidth = 1.)

	sc4 = ax[-1].scatter(array_smap, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038))  ,s = 5 , c='dodgerblue',marker='o', facecolors='dodgerblue',label='CLM SMAP_raw')
	ax[-1].plot(array_smap, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038)),c='dodgerblue', linestyle =':', linewidth = 1.) 

	sc5 = ax[-1].scatter(array_smapDailyCVEX, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038))  ,s = 5 , c='forestgreen',marker='h', facecolors='forestgreen',label='CLM SMAP_KF')
	...
	if gs2counter not in [0,5,10,15,20]:
...
#	ax[-1].set_xticklabels(ax[-1].xaxis.get_majorticklabels(),rotation=-45)
	ax[-1].tick_params(axis='x', which='major', labelsize=7, labelcolor='k', rotation=-30)
	...
	if gs2counter == 10:
		ax[-1].set_ylabel('Soil Depth (m)', fontsize=12, color='k')
		# ax[-1].yaxis.set_label_coords(-0.16, -0.15) 
	if gs2counter == 22:			
		ax[-1].set_xlabel('Soil Moisture Content ' + r'($\mathrm{mm^3/mm^3}$)', fontsize=12, color='k')
...
axe = fig.add_axes([0.1,0.0,0.8,0.03])	
pie = plt.pie([1,1])
lgnd = plt.legend(handles=[sc1,sc2,sc3,sc4,sc5],bbox_to_anchor=(0., 1.02, 1., .102),ncol=4, mode="expand", borderaxespad=0., edgecolor='lightgrey',prop={'size':10})
lgnd.legendHandles[0]._sizes = [70]  # size of the scatter in the legend
...
savefig( figDIR + 'FigS5_Ave_SM_vertical_X021_X024_X024kf_JJA_' + sDate_V + '_' + eDate_V + '_v3.png', bbox_inches='tight', dpi=600 )
savefig( figDIR + 'FigS5_Ave_SM_vertical_X021_X024_X024kf_JJA_' + sDate_V + '_' + eDate_V + '_v3.pdf', bbox_inches='tight')
