...
SMAPBC_IRRG_DIR = '/mnt/scratch/felfelan/CESM_simulations/X_024_centralUSA_ICRUCLM45BGCCROPNLDAS_SMAPdailyirr_KF/grndOBS_BiasCorrection_minIRR_tarSMAPoutput/'
...
for yy in range(sYEAR, eYEAR + 1):
    for mm in range(sMONTH, eMONTH + 1):
        ...
        smapBC_IRRG_FILE = netcdf.netcdf_file(SMAPBC_IRRG_DIR + 'X_024_centralUSA_ICRUCLM45BGCCROPNLDAS_SMAPdailyirr_KF.clm2.h0.' + str(yy).zfill(4) + '-' + str('%02d'%(mm) ) + '.nc','r')
        ...
        tarSMAPBC_h2osoi_dat_dummy = smapBC_IRRG_FILE.variables['H2OSOI_tarSMAP'][0,0:10,].reshape(10,400,520).mean(0)
        ...
        if yy == sYEAR and mm == sMONTH:
            ...
            tarSMAPBC_h2osoi_dat = tarSMAPBC_h2osoi_dat_dummy.reshape(1,400,520)
        else:
            ...
            tarSMAPBC_h2osoi_dat = np.ma.concatenate([tarSMAPBC_h2osoi_dat, tarSMAPBC_h2osoi_dat_dummy.reshape(1,400,520)], axis = 0)
...
tarSMAPBC_h2osoi_dat = tarSMAPBC_h2osoi_dat.mean(0)
tarSMAPBC_h2osoi_dat = flipud(tarSMAPBC_h2osoi_dat)
#=================================== read CLM data; Daily
xx = 0
...
xx = 0
for file in sorted(os.listdir(SMAPBC_IRRG_DIR)):
	if fnmatch.fnmatch(file, '*00000.nc'):
		smapBC_FILE = netcdf.netcdf_file(SMAPBC_IRRG_DIR + file,'r')
		nt = len(smapBC_FILE.variables['QIRRIG'][:])
		# take the average for the first 10 layers 
		tarSMAPBC_dat_dummy = np.flip(np.ma.masked_greater(smapBC_FILE.variables['H2OSOI_tarSMAP'][:,0:10,],1.0).mean(1),axis=1)
		
		mcdatBC = smapBC_FILE.variables['mcdate'][:]
		print(file)
		if xx == 0:
			tarSMAPBC_dat = tarSMAPBC_dat_dummy
			mcdateBC = mcdatBC 
			xx = 1.0
		else:
			tarSMAPBC_dat = np.ma.concatenate([tarSMAPBC_dat, tarSMAPBC_dat_dummy], axis = 0)
			mcdateBC = concatenate((mcdateBC,mcdatBC))		
...
smapBC_h2osoi_zero0 = zeros ( ( 1, 400, 520 ) )	
...
for i in range(400):
	...
	# if AQdat_hlf[i,j] == 1:
        if tar_h2osoi_dat[i,j] >= 0 and tar_h2osoi_dat[i,j] <= 1:
            ...
            smapBC_h2osoi_zero0[0,i,j] = tarSMAPBC_h2osoi_dat[i,j]

h2osoi_zero = np.ma.concatenate([ctrl_h2osoi_zero0,smap_h2osoi_zero0,smapBC_h2osoi_zero0], axis = 0)
...
gs2.update(bottom=0.11, top=0.53,wspace=0.17)  
...
title = ['(a) CTRL Target SM', '(b) SMAP_KF Target SM', '(c) SMAP_KF_BC Target SM']
text = ['d','e','f','g','h','i']
...
#	map1.drawstates(linewidth=0.25, color='grey')
	...
	cs = map1.imshow(ma.masked_greater_equal(ma.masked_less_equal(h2osoi_zero[ii,:,:cntralUSA3minxy(30,-92)[0]],0.0),1.0),origin='upper',interpolation='nearest',cmap=cmap,norm=norm, vmin=0.0)
...
#	ax.text(0.3, 0.85, text[ii], verticalalignment='bottom', horizontalalignment='right', transform=ax.transAxes,fontsize=20)
	plt.title(title[ii],fontsize=9)
	cbar_ax = fig.add_axes([0.2, 0.6, 0.6, 0.015])    
	cb = fig.colorbar(cs, cax=cbar_ax, cmap=cmap, norm=norm,spacing='uniform', ticks=bounds_b,orientation='horizontal', extend='max')
	cb.set_label('$mm^3/mm^3$',fontsize=9,labelpad=-22, x=1.1)
...
for jj in range(len(lats)):
	...
	df_TS = pd.DataFrame(columns=['Date','ctrl_tar','smap_tar','smapBC_tar' ])
	...
	df_TS['smapBC_tar'] = tarSMAPBC_dat[:,PointLatLon[1],PointLatLon[0]]
	...
	sc3 = ax.scatter(df_TS.index,df_TS['smapBC_tar'] ,s = 2 , c = 'lime', marker = "h", label = 'SMAP_KF_BC Target SM')
	...
	if jj == 0:
		lgnd = plt.legend(handles=[sc1,sc2,sc3],ncol=1, loc = 'lower left',bbox_to_anchor = (0.05,0.15), borderaxespad=0., edgecolor='darkgray',prop={'size':7})
		lgnd.legendHandles[0]._sizes = [50]  # size of the scatter in the legend
		lgnd.legendHandles[1]._sizes = [50]
		lgnd.legendHandles[2]._sizes = [50]
	if jj == 0:
		ax.set_ylabel('Target SM '+ r'($\mathrm{mm^3/mm^3}$)', fontsize=9, color='k')
		ax.yaxis.set_label_coords(-0.16, -0.15)
	plt.setp(ax.spines.values(), color='darkgray')
	
	
#savefig(figDIR + 'FigS2_targetSM_v4.png', dpi=600, bbox_inches='tight')
#savefig(figDIR + 'FigS2_targetSM_v4.pdf', bbox_inches='tight')
#
#plt.close()
