...
smap3min = np.ma.masked_equal(load(SMAP_DIR + 'SMAPregridded_3minCentralUS_daily_for_' +\
								    str(sYEAR) + '_' + str(sMONTH).zfill(2) + '.npy'), -9999.0)
print('SMAPregridded_3minCentralUS_daily_for_' + str(sYEAR) + '_' + str(sMONTH).zfill(2))
#=================================== read CLM data
xx = 0 
for file in sorted(os.listdir(clm_noCRnoIR)):
#	print(file)
#	if fnmatch.fnmatch(file, '*00000.nc') and (str(sYEAR) in file) and\
#							 (int(file.split('-')[1]) <= sMONTH +1) and (int(file.split('-')[1]) >= sMONTH -1):

	if fnmatch.fnmatch(file, '*00000.nc') and (str(sYEAR) in file) and\
							 (int(file.split('-')[1]) == sMONTH):
					 
		
		print(file)
		clm_noIR_FILE = netcdf.netcdf_file(clm_noCRnoIR + file,'r')
		clm_noIR_dat0      = clm_noIR_FILE.variables['H2OSOI'][:]
		mcdat              = clm_noIR_FILE.variables['mcdate'][:]
		clm_noIR_dat01     = clm_noIR_dat0[:,0,:,:]
		clm_noIR_dat02     = clm_noIR_dat0[:,1,:,:]
...
df['Date'] = pd.to_datetime(mcdate,format='%Y%m%d')
DateIndex = df[df.Date == str(sYEAR) + str(sMONTH).zfill(2) + str(Day).zfill(2)].index[0]

clm_noIR_dat = clm_noIR_dat[DateIndex]
...
diff3 = smap3min[Day-1,:,:cntralUSA3minxy(30,-92)[0]] - clm_noIR_zero0[0,:,:cntralUSA3minxy(30,-92)[0]]
...
plt.title('SMAP_Satellite - NOirrig ' + str(sYEAR) + '-' + str(sMONTH).zfill(2) + '-' + str(Day).zfill(2),fontsize=15)
...
savefig(figDIR + 'FigR1_CLMNOirrig_SMAP_centralUSA_5cmSM_Day_' + str(Day).zfill(2) + '_' + str(sYEAR) + '_' + str(eMONTH).zfill(2) + '.png', dpi=300, bbox_inches='tight')
