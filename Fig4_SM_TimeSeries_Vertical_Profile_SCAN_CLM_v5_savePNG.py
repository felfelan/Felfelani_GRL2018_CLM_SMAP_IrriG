...
gs1.update(bottom=0.39, top=0.95,wspace=0.12, hspace = 0.10)
...
gs2.update(bottom=0.05, top=0.3, wspace=0.15)

fig = plt.figure(num=1, figsize=(25.5,33)) #figsize: w,h letter size
ax = []
#%%======================================================================= read SCAN data
...
for cc in range(len(latseries)):
    ...
    df_SMAP_KF_h2osoi_5cmT['SMAP_KF_qflx_rain_grnd_' + st_names[cc]] = QFLX_RAIN_GRND_dat
...
#                           label='SCAN/CRN Observations')
    ...
    sc1 = ax[-1].scatter(df_JJA.index,df_JJA[st_names[cc]] ,s = 20 , c='black',marker="s",\
...
#    ax[-1].scatter(df_JJA.index, df_JJA[clmsmapKF_col].copy().mean(1),s = 5, c='forestgreen',label='CLM SMAP_KF')  <
    ...
    sc2 = ax[-1].scatter(df_JJA.index, df_JJA['ctrl_h2osoi_5cm_' + st_names[cc]].copy(),s = 15 , c='tomato',marker="^",label='CTRL')
    sc3 = ax[-1].scatter(df_JJA.index, df_JJA['SMAP_h2osoi_5cm_' + st_names[cc]].copy(),s = 15, c='dodgerblue',marker="o",label='SMAP_raw')
    sc4 = ax[-1].scatter(df_JJA.index, df_JJA['SMAP_KF_h2osoi_5cm_' + st_names[cc]].copy(),s = 15, c='forestgreen',marker="h",label='SMAP_kf_BC/SMAP_kf')

    ln2 = ax[-1].plot(df_JJA.index, df_JJA['ctrl_h2osoi_5cm_' + st_names[cc]].copy(),linewidth=1.0,c='tomato',label='CTRL')
    ln3 = ax[-1].plot(df_JJA.index, df_JJA['SMAP_h2osoi_5cm_' + st_names[cc]].copy(),linewidth=1.0,c='dodgerblue',label='SMAP_raw')
    ln4 = ax[-1].plot(df_JJA.index, df_JJA['SMAP_KF_h2osoi_5cm_' + st_names[cc]].copy(),linewidth=1.0,c='forestgreen',label='SMAP_kf_BC/SMAP_kf')
    ...
    ba2 = ax2.bar(df_JJA.index,df_JJA['SMAP_KF_qflx_rain_grnd_' + st_names[cc]],color='deepskyblue',width=1.0,bottom=df_JJA['SMAP_KF_qirrig_' + st_names[cc]],alpha=0.50,label='Precipitation SMAP_kf_BC')
    ...
    if cc == 3: #or cc ==2:    
        ax[-1].set_xticklabels(df_JJA.Date[0:len(df_JJA.index):10], fontsize=17, rotation=-45)
...
#        ax[-1].legend_.remove()
    ...
    if cc == 2:
        ax[-1].set_ylabel('Soil Moisture Content ' + r'($\mathrm{mm^3/mm^3}$)', fontsize=20, color='k')
        ax[-1].yaxis.set_label_coords(-0.04, 1.0) 
    ...
    ax[-1].text(0.08, 0.85, '(' + letter1[cc] + ')', verticalalignment='bottom', horizontalalignment='right', transform=ax[-1].transAxes,fontsize=40)    
    
    plt.xlim(df_JJA.index.min(),df_JJA.index.max())
    # plt.ylim(0,0.5)
    plt.title('Top 5cm Soil Moisture for JJA - ' +  st_names[cc],fontsize=20)    
    
    if cc == 2:
        ax2.set_ylabel('Irrigation Water Amount ' + r'($\mathrm{mm/day}$)', fontsize=20, color='k')
        ax2.yaxis.set_label_coords(1.035, 1.0)    

axe = fig.add_axes([0.11,0.29,0.8,0.03])	
pie = plt.pie([1,1])
...
lgnd = plt.legend(handles=[sc1,sc2,sc3,sc4,sc_fake,ba1, ba2],bbox_to_anchor=(0., 1.02, 1., .102),ncol=4, mode="expand", borderaxespad=0., edgecolor='k',prop={'size':20})
lgnd.legendHandles[0]._sizes = [100]  # size of the scatter in the legend
...
#%%======================================================================= Plotting the Whole timeseries
	...
	ax[-1].scatter(SMAP_monthly,0.025 ,s = 200 , c='black',marker='*', facecolors='black',label='SMAP')

	if 'SCAN' in tables:	
		ax[-1].scatter(array_scan, array((0.05,0.1,0.2,0.5,1))  ,s = 20 , c='black',marker='s', facecolors='black',label='SCAN')
	else:
		ax[-1].scatter(array_scan, array((0.05,0.1,0.2,0.5,1))  ,s = 20 , c='black',marker='s', facecolors='black',label='USCRN')

	ax[-1].plot(array_scan, array((0.05,0.1,0.2,0.5,1)),c='black', linestyle =':', linewidth = 2)
	
	ax[-1].scatter(array_ctrl, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038))  ,s = 20 , c='tomato',marker='^', facecolors='tomato',label='CLM_ctrl')
	ax[-1].plot(array_ctrl, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038)),c='tomato', linestyle =':', linewidth = 1.5)

	ax[-1].scatter(array_smap, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038))  ,s = 20 , c='dodgerblue',marker='o', facecolors='dodgerblue',label='CLM SMAP_raw')
	ax[-1].plot(array_smap, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038)),c='dodgerblue', linestyle =':', linewidth = 1.5) 

	ax[-1].scatter(array_smapDailyCVEX, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038))  ,s = 20 , c='forestgreen',marker='h', facecolors='forestgreen',label='CLM SMAP_kf')
	ax[-1].plot(array_smapDailyCVEX, array((0.0071, 0.0279, 0.0623, 0.1189, 0.2122, 0.3661, 0.6198, 1.038)),c='forestgreen', linestyle =':', linewidth = 1.5) 	
	...
	ax[-1].set_title(TABLE_Both, fontsize=20)
	if gs2counter == 0:
		ax[-1].set_ylabel('Soil Depth (m)', fontsize=20, color='k')
		ax[-1].yaxis.set_label_coords(-0.16, -0.15) 
	if gs2counter == 6:			
		ax[-1].set_xlabel('Soil Moisture Content ' + r'($\mathrm{mm^3/mm^3}$)', fontsize=20, color='k')
	...
	ax[-1].text(0.22, 0.1, '(' + letter2[gs2counter] + ')', verticalalignment='bottom', horizontalalignment='right', transform=ax[-1].transAxes,fontsize=40)
	gs2counter = gs2counter + 1 
  
savefig( figDIR + 'Fig4_SM_timeseriesLHPA_' + sDate_T + eDate_T + '_Ave_SM_vertical_X021_X024_X024kf_JJA' + '_' + sDate_V + '_' + eDate_V + '_v6.png', bbox_inches='tight', dpi=600 )
# savefig( figDIR + 'Fig4_SM_timeseriesLPHA_' + sDate_T + eDate_T + '_Ave_SM_vertical_X021_X024_X024kf_JJA' + '_' + sDate_V + '_' + eDate_V + '_v6.pdf', bbox_inches='tight')
